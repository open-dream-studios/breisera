...
export const handle1XCheckoutTransaction = async (event, res) => {
...
export const handleSubscriptionCheckoutTransaction = async (
  ...
  try {
    ...
    if (stripe_subscription_id) {
      ...
      stripe_current_period_start = formatDateForMySQL(
        subscription.current_period_start
      );
      stripe_current_period_end = formatDateForMySQL(
...
export const updateUserCredits = async (credits, user_id) => {
  const user = await new Promise((resolve, reject) => {
    db.query(
      "SELECT * FROM users WHERE user_id = ?",
      [user_id],
      (err, data) => {
        if (err) {
          console.error(
            "DB Query Error: Could not fetch user",
            err
          );
          return reject(err);
        }
