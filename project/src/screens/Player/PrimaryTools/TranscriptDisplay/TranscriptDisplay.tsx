          ...
          {[1, 2, 3, 4, 5].map((item: number, index: number) => {
            return (
              <div className="flex flex-col gap-[9px]" key={index}>
                <Skeleton className={`w-[100px] h-[25px] ${currentUser.theme === "dark" ? "brightness-25" : "brightness-[90%]"}`} />
                <Skeleton className={`w-[50%] h-[25px] mb-[20px] ${currentUser.theme === "dark" ? "brightness-25" : "brightness-[90%]"}`} />
              </div>
            );
          })}
        </>
